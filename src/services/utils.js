export function cpfMask(value) {
  ...
  export function toFormatCurrency(value, decimalPlaces = 2) {
    return new Intl.NumberFormat("pt-BR", {
      style: "currency",
      currency: "BRL",
      minimumFractionDigits: decimalPlaces, // Define o mínimo de casas decimais
      maximumFractionDigits: decimalPlaces, // Define o máximo de casas decimais
    }).format(value);
  }
  ...
  export function toFormatNumberAsNumber(value, digits) {
    const decimalPlaces = Math.pow(10, digits);
  ...
  export function isCpf(c) {
    ...
    if (c == "11111111111") return false;
  
    var r;
    var s = 0;
  
    for (var i = 1; i <= 9; i++) s = s + parseInt(c[i - 1]) * (11 - i);
  ...
  export function toFormatDateYYYYMMDD3(date) {
    var d = new Date(date);
    // d = d.toLocaleDateString('pt-BR', {timeZone: 'UTC'})
    d = toFormatDate(date);
    var dd = d.split("/");
  ...
  export function nFormattertoCurrency(num, digits) {
    var si = [
      { value: 1, symbol: "" },
    ...
    return (
      toFormatCurrency((num / si[i].value).toFixed(digits).replace(rx, "$1")) +
      si[i].symbol
    );
  }
